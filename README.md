...
- **shebang 문법 자동완성** - `{{{#!` 입력시, `{{{#!`(shebang)가 앞으로 오는 문법 계열인 `#!wiki #!folding #!html #!syntax #!if #!style`을 자동완성으로 띄워줍니다.
...
- **#!wiki 매개변수 자동완성** - `{{{#!wiki ` 입력시, `style, dark-style, class, lang`을 자동완성으로 띄워줍니다.
- **#!wiki 매개변수 세부 자동완성** - `{{{#!wiki style="` 등 wiki 매개변수를 사용하여 입력시, 맥락에 맞는 자동완성을 띄워줍니다. `style`과 `dark-style`은 css의 속성과 허용값, `class`는 `#!style`에 사용된 class 목록, `lang`은 BCP 47에 따른 간단한 언어 코드 목록을 자동완성으로 띄워줍니다.
...
- **선택 영역 구문 감싸기: {{{...}}} 구문**
  - 일반 삼중괄호, `wiki`, `if`, `syntax`, `folding`, `style`, `html` 구문으로 선택한 내용을 감쌀 수 있습니다.
- **현재 위치한 구문 벗기기: {{{...}}} 구문**
